...
// Remove after initial data migration is complete!
// ════════════════════════════════════════════════════════════

// Upload full database (replaces data.json)
router.post('/migrate/data', adminOnly, (req, res) => {
  if (!req.body || !req.body.users) return res.status(400).json({ error: 'Invalid database format' });
  const { saveCritical } = require('../database');
  saveCritical(req.body);
...
router.post('/migrate/upload', adminOnly, (req, res) => {
  const { filename, data } = req.body;
  if (!filename || !data) return res.status(400).json({ error: 'filename and data required' });

  const UPLOAD_DIR = process.env.UPLOAD_DIR || path.join(__dirname, '..', 'public', 'uploads');
  const newsDir = path.join(UPLOAD_DIR, 'news');
  if (!fs.existsSync(newsDir)) fs.mkdirSync(newsDir, { recursive: true });

  const buffer = Buffer.from(data, 'base64');
  fs.writeFileSync(path.join(newsDir, filename), buffer);
  res.json({ ok: true, path: '/uploads/news/' + filename, size: buffer.length });
});

// Check migration status
router.get('/migrate/status', adminOnly, (req, res) => {
  const db = require('../database').load();
  const UPLOAD_DIR = process.env.UPLOAD_DIR || path.join(__dirname, '..', 'public', 'uploads');
  const newsDir = path.join(UPLOAD_DIR, 'news');
  const uploads = fs.existsSync(newsDir) ? fs.readdirSync(newsDir).filter(f => !f.startsWith('.')) : [];
  res.json({
